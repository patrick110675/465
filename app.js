...
function bindNav(){document.querySelectorAll('[data-page]').forEach(b=>b.onclick=()=>showPage(b.dataset.page));document.querySelectorAll('[data-admin-tab]').forEach(b=>b.onclick=()=>{showPage('admin');currentAdmin=b.dataset.adminTab;renderAdmin();});document.querySelectorAll('.admin-tab').forEach(b=>b.onclick=()=>{currentAdmin=b.dataset.admin;renderAdmin();});document.querySelectorAll('.tab').forEach(b=>b.onclick=()=>{document.querySelectorAll('.tab').forEach(x=>x.classList.remove('active'));b.classList.add('active');rankMode=b.dataset.rank;renderTop5();});document.getElementById('quickSearch').oninput=e=>quickSearch(e.target.value);document.addEventListener('click',e=>{if(!e.target.closest('.quick-search-wrap'))document.getElementById('quickSearchResults')?.setAttribute('hidden','');if(!e.target.closest('.autocomplete-field'))document.querySelectorAll('.autocomplete-menu').forEach(x=>x.hidden=true);});}
...
function archiveCompetition(){const active=state.competitions.find(c=>c.active!==false)||{};const name=active.name||state.settings.banner||'目前競賽';const start=active.start||'未設定';const end=active.end||'未設定';const periodSales=state.sales.filter(s=>(!active.start||s.date>=active.start)&&(!active.end||s.date<=active.end));const ranking=aggregate(periodSales,'userName').sort((a,b)=>b.contestWeighted-a.contestWeighted);const same=state.history.some(h=>h.competitionName===name&&h.start===active.start&&h.end===active.end);let message=`即將封存：${name}\n期間：${start}～${end}\n報件：${periodSales.length} 筆\n人員：${ranking.length} 位\n\n封存只是保存快照，不會刪除原始報件。確定繼續？`;if(same)message+=`\n\n⚠️ 此競賽期間已有封存紀錄，再次封存會新增另一份快照。`;if(!confirm(message))return;const snapshot={id:uid(),time:new Date().toLocaleString('zh-TW'),competitionName:name,start:active.start||'',end:active.end||'',banner:state.settings.banner,ranking,sales:JSON.parse(JSON.stringify(periodSales))};state.history.unshift(snapshot);log('封存競賽',`${name} ${start}～${end}`);save();renderHistory();toast('已封存競賽快照');}
...
function quickSearch(q){const box=document.getElementById('quickSearchResults');const key=norm(q).toLowerCase();if(!key){box.hidden=true;box.innerHTML='';return;}const people=state.users.filter(u=>[u.name,u.unit,u.team,u.group,u.role].some(v=>String(v||'').toLowerCase().includes(key))).slice(0,6);const products=state.products.filter(p=>[p.name,p.code,p.category,p.subcategory,p.year].some(v=>String(v||'').toLowerCase().includes(key))).slice(0,6);const bonus=state.bonus.filter(b=>[b.name,b.category,b.products].some(v=>String(v||'').toLowerCase().includes(key))).slice(0,4);const rows=[...people.map(x=>({type:'user',value:x.name,label:`👤 ${x.name}｜${x.unit||''}｜${x.role||''}`})),...products.map(x=>({type:'product',value:x.name,label:`📦 ${x.name}${x.code?`｜${x.code}`:''}｜${x.category||''}`})),...bonus.map(x=>({type:'bonus',value:x.name,label:`🎁 ${x.name}`}))];box.innerHTML=rows.map(r=>`<button type="button" data-search-type="${r.type}" data-search-value="${escapeHtml(r.value)}">${escapeHtml(r.label)}</button>`).join('')||'<div class="autocomplete-empty">找不到符合資料</div>';box.hidden=false;box.querySelectorAll('[data-search-type]').forEach(b=>b.onclick=()=>{const type=b.dataset.searchType,value=b.dataset.searchValue;if(type==='user'){showPage('sales');document.getElementById('saleUser').value=value;document.getElementById('salePremium').focus();}else if(type==='product'){showPage('sales');document.getElementById('saleProduct').value=value;document.getElementById('salePremium').focus();}else{showPage('dashboard');document.getElementById('bonusCards')?.scrollIntoView({behavior:'smooth',block:'center'});}document.getElementById('quickSearch').value='';box.hidden=true;});}
