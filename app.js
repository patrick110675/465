...
const demo={
  settings:{banner:'115下半年 新高峰競賽',period:'2026/07/02 - 2026/12/15',officeTarget:40000000,theme:{bg:'#eef7ff',primary:'#1769e8',text:'#0b1b3d',card:'#ffffff',radius:18}},
  users:[
...
function renderDashboard(){
  ...
  const thisMonth=state.sales.filter(s=>monthKey(s.date)===monthKey(today())); const done=sum(thisMonth,'contestWeighted'), target=Number(state.settings.officeTarget)||40000000; officeTarget.textContent=fmt(target); officeDone.textContent=fmt(done); officeRemain.textContent=fmt(Math.max(target-done,0)); officeRate.textContent=pct(done/target); officeBar.style.width=`${Math.min(done/target*100,100)}%`;
  const me=state.users.find(u=>u.name==='張永朋')||state.users[0]; const mySales=thisMonth.filter(s=>s.userName===me.name); const myDone=sum(mySales,'contestWeighted'); const pTarget=(state.competitions.find(c=>c.name==='新高峰'&&c.role===me.role)?.weightedTarget)||2650000; const sTarget=(state.competitions.find(c=>c.name==='新極峰'&&c.role===me.role)?.weightedTarget)||4000000; peakRate.textContent=pct(myDone/pTarget); peakBar.style.width=`${Math.min(myDone/pTarget*100,100)}%`; peakRemain.textContent=`還差 ${fmt(Math.max(pTarget-myDone,0))}`; superRate.textContent=pct(myDone/sTarget); superBar.style.width=`${Math.min(myDone/sTarget*100,100)}%`; superRemain.textContent=`還差 ${fmt(Math.max(sTarget-myDone,0))}`;
  renderBonus(mySales); renderTop5(); renderLatest();
}
...
function displayField(v,f){if(['originalWeight','contestWeight'].includes(f))return weightText(v);if(typeof v==='boolean')return v?'是':'否';if(['weightedTarget','premiumTarget','target','amount','usd'].includes(f))return fmt(v);return v??''}
