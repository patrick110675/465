...
function migrateCompetitions(rows){
  ...
  rows.forEach(r=>{
    ...
    if(!groups.has(key))groups.set(key,{id:uid(),name:r.name||'未命名競賽',start:r.start||'',end:r.end||'',logic:'AND',active:true,reward:r.reward||'',metrics:{weighted:{enabled:true},premium:{enabled:false},ah:{enabled:false}},targets:{},extraConditions:[]});
...
function normalizeCompetition(c){
  return {id:c.id||uid(),name:c.name||'未命名競賽',start:c.start||'',end:c.end||'',logic:c.logic==='OR'?'OR':'AND',active:c.active!==false,reward:c.reward||'',metrics:{weighted:{enabled:c.metrics?.weighted?.enabled!==false},premium:{enabled:!!c.metrics?.premium?.enabled},ah:{enabled:!!c.metrics?.ah?.enabled}},targets:c.targets||{},extraConditions:Array.isArray(c.extraConditions)?c.extraConditions:[]};
}
...
function competitionStatus(c,user){
  ...
  ['weighted','premium','ah'].forEach(metric=>{if(!c.metrics?.[metric]?.enabled)return;const target=competitionTarget(c,user?.role,metric);const done=competitionMetricValue(metric,sales);rows.push({metric,target,done,rate:target?Math.min(done/target*100,100):0,met:target>0&&done>=target});});
...
function renderCompetitionProgress(me){
  ...
  const comps=state.competitions.filter(c=>c.active!==false);
...
function renderDashboardSettings(p){
  ...
  p.querySelector('#saveDashboardSettings').onclick=()=>{const v=id=>p.querySelector('#'+id);state.settings.officeTarget=Number(v('officeTargetSetting').value)||0;state.settings.officeDoneManual=Number(v('officeDoneSetting').value)||0;state.settings.officeManual=v('officeManualSetting').checked;state.settings.peakTargetManual=Number(v('peakTargetSetting').value)||0;state.settings.peakDoneManual=Number(v('peakDoneSetting').value)||0;state.settings.superTargetManual=Number(v('superTargetSetting').value)||0;state.settings.superDoneManual=Number(v('superDoneSetting').value)||0;state.settings.competitionManual=v('competitionManualSetting').checked;save();renderDashboard();toast('通訊處與競賽業績已更新');};
