...
function readImportFile(e){const file=e.target.files[0];if(!file)return;const reader=new FileReader();reader.onload=ev=>{try{const wb=XLSX.read(ev.target.result,{type:'array',cellDates:true});const selected=chooseImportSheet(wb,importType.value);pendingImport=selected.rows;pendingImportMeta={sheetName:selected.name,fileName:file.name,type:importType.value,mapped:[]};renderImportPreview();}catch(err){console.error(err);pendingImport=[];importPreview.innerHTML=`<div class="card"><b>⚠️ 無法讀取檔案</b><p>${escapeHtml(err.message||'Excel 格式錯誤')}</p></div>`;confirmImport.disabled=true;}};reader.readAsArrayBuffer(file);}
...
function findImportUser(name){const n=norm(name);return state.users.find(u=>norm(u.name)===n)||state.users.find(u=>norm(u.name).replace(/\s/g,'')===n.replace(/\s/g,''));}
function findImportProduct(name,code){const n=norm(name),c=norm(code).toUpperCase();return state.products.find(p=>c&&norm(p.code).toUpperCase()===c)||state.products.find(p=>n&&norm(p.name)===n)||state.products.find(p=>n&&(norm(p.name).includes(n)||n.includes(norm(p.name))));}
